 ...
 */
inline bool isValid(uint32_t magic) {
    return magic == MAGIC;
}
...
inline const Tag* findTag(uint32_t multibootAddr, TagType tagType) {
    ...
    while (true) {
        ...
        if (tag->type == static_cast<uint32_t>(tagType)) {
            return tag;
        }
 ...
 */
inline const TagFramebuffer* findFramebuffer(uint32_t multibootAddr) {
    return reinterpret_cast<const TagFramebuffer*>(findTag(multibootAddr, TagType::Framebuffer));
}

/**
 * @brief Find basic memory info tag
 */
inline const TagBasicMemInfo* findBasicMemInfo(uint32_t multibootAddr) {
    return reinterpret_cast<const TagBasicMemInfo*>(findTag(multibootAddr, TagType::BasicMemInfo));
}

/**
 * @brief Find memory map tag
 */
inline const TagMemoryMap* findMemoryMap(uint32_t multibootAddr) {
    return reinterpret_cast<const TagMemoryMap*>(findTag(multibootAddr, TagType::MemoryMap));
}
...
inline uint64_t getTotalAvailableMemory(uint32_t multibootAddr) {
    ...
    if (!mmap) {
        ...
        if (basic) {
            return (static_cast<uint64_t>(basic->mem_upper) + 1024) * 1024;
        }
        return 0;
    }
