...
bool isKernelInitialized() {
    return g_kernelInitialized;
}
...
bool initKernel(uint32_t magic, uintptr_t multibootAddr) {
    ...
    LOG_INFO("Framebuffer: %ux%u, %u bpp, pitch=%u, addr=0x%x", 
             g_kernel.fbWidth, g_kernel.fbHeight, g_kernel.fbBpp, 
             g_kernel.fbPitch, static_cast<uint32_t>(g_kernel.fbAddress));
    ...
    LOG_DEBUG("Back buffer allocated at 0x%x (%u frames)", 
              reinterpret_cast<uintptr_t>(g_kernel.backBuffer), static_cast<uint32_t>(backBufferFrames));
...
[[noreturn]] void kernelMain() {
    ...
    text.setCursor(0, 0);
    text << Color::Gray400 << "Display: " << Color::White << k.fbWidth << "x" << k.fbHeight 
         << Color::Gray400 << " @ " << Color::White << k.fbBpp << " bpp\n"
    ...
    text << Color::Yellow << "=== Memory Layout ===\n"
         << Color::Gray400 << "Physical Memory: " << Color::White 
         << static_cast<uint32_t>(PhysicalMemory::getFreeMemory() / (1024 * 1024)) << " MB free / "
         << static_cast<uint32_t>(PhysicalMemory::getTotalMemory() / (1024 * 1024)) << " MB total\n"
         << Color::Gray400 << "Kernel:  " << Color::White 
         << HEX() << static_cast<uint32_t>(arch::boot::getKernelStart()) << " - " 
         << static_cast<uint32_t>(arch::boot::getKernelEnd()) << DEC()
         << Color::Gray400 << " (" << Color::White << static_cast<uint32_t>(arch::boot::getKernelSize() / 1024) << " KB" << Color::Gray400 << ")\n"
         << Color::Gray400 << "Stack:   " << Color::White 
         << HEX() << static_cast<uint32_t>(arch::boot::get_stack_bottom()) << " - " 
         << static_cast<uint32_t>(arch::boot::get_stack_top()) << DEC()
         << Color::Gray400 << " (" << Color::White << static_cast<uint32_t>(arch::boot::get_stack_size() / 1024) << " KB" << Color::Gray400 << ")\n"
         << Color::Gray400 << "Sections: .text " << Color::White << static_cast<uint32_t>(arch::boot::getTextSize() / 1024) << " KB"
         << Color::Gray400 << ", .data " << Color::White << static_cast<uint32_t>(arch::boot::getDataSize() / 1024) << " KB"
         << Color::Gray400 << ", .bss " << Color::White << static_cast<uint32_t>(arch::boot::getBssSize() / 1024) << " KB\n";
    ...
    text << Color::Yellow << "=== Components ===\n"
         << Color::Cyan << "SDK: " 
         << Color::White << "Color, Font, FrameBuffer, Brush, TextRenderer\n"
         << Color::Cyan << "Kernel: "
         << Color::White << "Display, Logger, Panic, Timer, IO, PhysicalMemory\n"
         << Color::Cyan << "Libc: "
         << Color::White << "memory, string, stdio, stdlib, ctype, math\n";
