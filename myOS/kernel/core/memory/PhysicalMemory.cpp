...
void* kmalloc_early(size_t size) {
    ...
    if (s_placementAddress == 0) {
        s_placementAddress = reinterpret_cast<uintptr_t>(__kernel_end);
    }
...
void* kmalloc_early_aligned(size_t size, size_t align) {
    ...
    // Align the address
    if (s_placementAddress & (align - 1)) {
        s_placementAddress = (s_placementAddress + align) & ~(align - 1);
    }
...
void PhysicalMemory::initialize(uintptr_t kernelEnd, size_t memorySize) {
    ...
    LOG_INFO("Physical memory: %u MB total, %u MB free", 
             static_cast<uint32_t>(getTotalMemory() / (1024 * 1024)),
             static_cast<uint32_t>(getFreeMemory() / (1024 * 1024)));
