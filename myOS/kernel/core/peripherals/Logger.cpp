...
namespace myOS::kernel {
    ...
    bool Logger::waitForTxReady(uint32_t timeout) {
        for (uint32_t i = 0; i < timeout; ++i) {
            if (lsrPort_.read() & LSR_TX_EMPTY) { return true; }
        }
        return false;
    }
    ...
    void Logger::putChar(char ch) {
        ...
        if (waitForTxReady()) { dataPort_.write(static_cast<uint8_t>(ch)); }
    }

    void Logger::putString(const char* str) {
        if (!str) return;
        for (size_t i = 0; str[i] != '\0' && i < 4096; ++i) { putChar(str[i]); }
    }
    ...
    void Logger::printf(const char* format, ...) {
        ...
        for (size_t i = 0; format[i] != '\0'; ++i) {
            ...
            switch (format[i]) {
                case '%': putChar('%'); break;
                case 'c': putChar(static_cast<char>(va_arg(args, int))); break;
                case 's': {
    ...
    void log(LogLevel level, const char* function, uint32_t line, const char* format, ...) {
        ...
        for (size_t i = 0; format[i] != '\0'; ++i) {
            ...
            switch (format[i]) {
                case '%': g_logger->putChar('%'); break;
                case 'c': g_logger->putChar(static_cast<char>(va_arg(args, int))); break;
                case 's': {
                    const char* str = va_arg(args, const char*);
