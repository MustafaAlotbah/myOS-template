...
Brush::Brush(FrameBuffer& frameBuffer)
...
void Brush::drawPoint(uint32_t x, uint32_t y, Color color) {
    if (x < frameBuffer_.getWidth() && y < frameBuffer_.getHeight()) {
...
void Brush::fillRect(uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2, Color color) {
    ...
    if (x1 > x2) { uint32_t t = x1; x1 = x2; x2 = t; }
    if (y1 > y2) { uint32_t t = y1; y1 = y2; y2 = t; }
    ...
    for (uint32_t y = y1; y < y2; ++y) {
        ...
        for (uint32_t i = 0; i < rectWidth; ++i) {
            row[i] = colorValue;
        }
    }
}
