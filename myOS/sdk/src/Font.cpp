...
namespace myOS {
    ...
    Font::Font(const char* name) : name_(name) {
        ...
        for (int i = 0; i < MAX_FONT_CHARS; ++i) { glyphs_[i] = {{0}, 0, 0, 0, 0}; }
    }

    const Glyph& Font::getGlyph(uint32_t ch) const {
        if (ch >= MAX_FONT_CHARS) { return glyphs_[0]; }
        return glyphs_[ch];
    }

    void Font::setGlyph(uint32_t ch, const Glyph& glyph) {
        if (ch < MAX_FONT_CHARS) { glyphs_[ch] = glyph; }
    }

    void Font::initialize() { initializePoppins(); }

    void Font::initializePoppins() {
        for (int i = 0; i < 32; i++) { Poppins.setGlyph(i, {{0}, 0, 0, 0, 0}); }
