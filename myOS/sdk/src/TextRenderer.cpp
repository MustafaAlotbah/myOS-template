...
namespace myOS {
    ...
    TextRenderer::TextRenderer(FrameBuffer& fb, Font& font)
        : fb_(fb), font_(&font), textColor_(Color::White), numSystem_(NumeralSystem::Dec), precision_(3), cursorX_(0), cursorY_(0), positionX_(0), positionY_(0),
          width_(fb.getWidth()), height_(fb.getHeight()), lineSpacing_(3) {}
    ...
    void TextRenderer::setFont(Font& font) { font_ = &font; }

    void TextRenderer::setColor(Color color) { textColor_ = color; }

    void TextRenderer::setPrecision(uint8_t precision) { precision_ = precision; }
    ...
    uint32_t TextRenderer::calculateHeight() { return font_->getGlyph('A').height; }
    ...
    void TextRenderer::putChar(char ch) {
        ...
        // Handle line wrap
        if (cursorX_ + static_cast<int>(glyph.width) >= static_cast<int>(width_)) { newLine(); }
        ...
        for (uint8_t col = 0; col < glyph.width; ++col) {
            for (uint8_t row = 0; row < glyph.height; ++row) {
                if (glyph.bitmap[col] & (1 << row)) {
                    ...
                    if (x >= 0 && x < static_cast<int>(fb_.getWidth()) && y >= 0 && y < static_cast<int>(fb_.getHeight())) {
    ...
    void TextRenderer::putString(const char* str) {
        for (int i = 0; str[i] != '\0' && i < 4096; ++i) { putChar(str[i]); }
    }
    ...
    void TextRenderer::printNumber(uint64_t value, bool negative) {
        ...
        switch (numSystem_) {
            case NumeralSystem::Hex: base = 16; break;
            case NumeralSystem::Bin: base = 2; break;
    ...
    TextRenderer& TextRenderer::operator<<(Command command) {
        switch (command) {
            case Command::SwapBuffers: fb_.swapBuffers(); break;
            case Command::NewLine: newLine(); break;
        }
        return *this;
    }
